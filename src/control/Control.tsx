...
export function Control() {
  ...
  const useMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => setGeoMsg(err.code === err.PERMISSION_DENIED ? "Permission denied" : "Couldn't get location"),
    ...
    <div className="control">
      ...
      <main>
        <Section title="Location">
          <Row label="Latitude" hint="where to watch the sky">
            <Coord value={cfg.centerLat} step={0.01}
              onChange={(v) => set({ centerLat: clampLat(v) })} />
          </Row>
          <Row label="Longitude">
            <Coord value={cfg.centerLon} step={0.01}
              onChange={(v) => set({ centerLon: clampLon(v) })} />
          </Row>
          <div className="chips">
